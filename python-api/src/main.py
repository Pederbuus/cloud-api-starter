...
def db(query: str, fetch: str | None = None):
    """
    Kør et SQL-query på PostgreSQL databasen.

    Args:
        query (str): SQL-forespørgslen der skal køres.
        fetch (str | None): 
            - 'one' for at returnere én række,
            - 'all' for at returnere alle rækker,
            - None for kun at committe (INSERT, UPDATE, DELETE).

    Returns:
        Resultatet fra databasen eller None.
    """
    # Bepend on the docker-compose.yml?
    connection = psycopg2.connect(
        database="rust_api_db",
        user="user",
        password="password",
        host="db",
        port=5432
...
def read_total_vehicle():
    responce = db("SELECT COUNT(*) FROM vehicle;", "all")
    total = responce[0] if responce else 0
    return {total}
